...
namespace CKCharaDataEditor.Forms
{
    public partial class ConditionForm : Form
    {
        ...
        private void InitConditons()
        {
            ...
            int maxConditionId = Conditions.Count > 0 ? Conditions.Max(c => c.Id) : 0;
            if (maxConditionId > ConditionDescriptions.Last().ID)
            {
                ...
                string descriptionPath = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "ConditionDescription.csv")
        ...
        {
            string conditionDescriptionFilePath = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "ConditionDescription.csv")
                ?? throw new FileNotFoundException("ConditionDescription.csvが見つかりません。");
