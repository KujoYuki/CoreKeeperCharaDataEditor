...
namespace CKCharaDataEditor.Forms
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            string additionalIngredientsFilePath = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "AdditionalIngredients.csv")
        ...
        private void DeleteAbnormalRecipeButton_Click(object sender, EventArgs e)
        {
            var dialogResult = MessageBox.Show("ゲーム内では作成不可能な組み合わせのレシピを削除しますか？\n削除したレシピは戻りません。", "確認", MessageBoxButtons.OKCancel);
            if (dialogResult is DialogResult.OK)
            {
                int abnormalRecipeCount = _saveDataManager.DeleteAbnormalRecipes();
                MessageBox.Show($"作成不可能なレシピ {abnormalRecipeCount} 個の削除が完了しました。", "確認", MessageBoxButtons.OKCancel);
            }
        }
        ...
        private void IdOrKeyTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            string? key;
            if (int.TryParse(IdOrKeyTextBox.Text, out int objectId))
            {
                _fileManager.ObjectIdWithKey.TryGetValue(objectId, out key);
            }
            else
            {
        ...
        private void specialTextInputButton_Click(object sender, EventArgs e)
        {
            using(var detailInputForm = new SpecialStringInputForm())
            {
                detailInputForm.SpecialText = displayNameTextBox.Text;
                if (detailInputForm.ShowDialog() == DialogResult.OK)
                {
                    displayNameTextBox.Text = detailInputForm.SpecialText;
                }
            }
        }
    }
