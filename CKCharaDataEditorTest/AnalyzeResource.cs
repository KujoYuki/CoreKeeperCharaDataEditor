...
namespace CKCharaDataEditorTest
{
    public static class AnalyzeResource
    {
        ...
        // 全ての食材と料理の表示名を取得
        internal static readonly List<(int objectID, string DisplayName)> Ingredients = Food.AllIngredients
            .Concat(Food.ObsoleteIngredients)   // レシピには載らないが料理は作成できるため含める
            .Select(item => (item.objectID, item.DisplayName))
            .ToList();

        // Id=>DisplayNameの辞書
        internal static readonly Dictionary<int, string> FoodDic = Food.AllCookedBaseCategories
            .Select(item =>
            {
                return new[] { item.objectID, item.objectID + (int)CookRarity.Rare, item.objectID + (int)CookRarity.Epic }
                .Select(id => (objectID: id, item.DisplayName));
            })
